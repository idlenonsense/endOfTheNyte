...
public class EndenyteOreBlock
extends Block {
    public static final BooleanProperty LIT = RedstoneTorchBlock.LIT;

    public EndenyteOreBlock(AbstractBlock.Settings settings) {
        super(settings);
    ...
    public void onStacksDropped(BlockState state, ServerWorld world, BlockPos pos, ItemStack tool, boolean dropExperience) {
        ...
        if (dropExperience && EnchantmentHelper.getLevel(Enchantments.SILK_TOUCH, tool) == 0) {
            int i = 7 + world.random.nextInt(7);
    ...
    public void onSteppedOn(World world, BlockPos pos, BlockState state, Entity entity) {
        if (!entity.bypassesSteppingEffects()) {
            EndenyteOreBlock.light(state, world, pos);
        }
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        if (world.isClient) {
            EndenyteOreBlock.spawnParticles(world, pos);
        } else {
            EndenyteOreBlock.light(state, world, pos);
        }
    ...
    private static void spawnParticles(World world, BlockPos pos) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            world.addParticle(new DustParticleEffect(Vec3d.unpackRgb(0x86439D).toVector3f(), 1.0f), (double)pos.getX() + e, (double)pos.getY() + f, (double)pos.getZ() + g, 0.0, 0.0, 0.0);
        }
    }
